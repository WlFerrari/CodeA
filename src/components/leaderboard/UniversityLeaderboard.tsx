...
const UniversityLeaderboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        api.getUniversityLeaderboard()
            .then((res) => {
                if (!cancelled) {
                    const rankings = res.universities.map((uni, index) => ({
                        ...uni,
            ...
            .catch(() => {
                const storedUsers = JSON.parse(localStorage.getItem('academic_users') || '[]') as LeaderboardUser[];
                const universityData: { [key: string]: { totalScore: number; users: LeaderboardUser[] } } = {};
