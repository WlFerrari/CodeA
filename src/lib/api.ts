...
export const api = {
    health: () => http<{ ok: boolean }>(`/api/health`).catch(() => ({ ok: false })),
    upsertUser: (user: Partial<LeaderboardUser>) => http(`/api/users/upsert`, { method: 'POST', body: JSON.stringify(user) }),
    bulkUpsert: (users: Partial<LeaderboardUser>[]) => http(`/api/users/bulk-upsert`, { method: 'POST', body: JSON.stringify(users) }),
